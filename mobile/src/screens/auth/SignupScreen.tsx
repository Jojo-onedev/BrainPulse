...
export const SignupScreen = ({ navigation }: any) => {
    ...
    const handleSignup = async () => {
        ...
        try {
            const result = await signUp(email.trim().toLowerCase(), password, displayName, selectedPhoto || undefined);

            if (result.requiresConfirmation) {
                setAlert({
                    visible: true,
                    title: 'Vérifiez vos emails',
                    message: 'Un lien de confirmation a été envoyé à ' + email + '. Veuillez valider votre compte avant de vous connecter.',
                    type: 'success'
                });
                // Optional: navigate to Login after a delay or just let them click "OK"
            } else {
                navigation.reset({
                    index: 0,
